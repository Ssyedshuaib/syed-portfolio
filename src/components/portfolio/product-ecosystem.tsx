    ...
    <section id="ecosystem" className="py-48 px-6" ref={sectionRef}>
      <div className="max-w-7xl mx-auto space-y-24">
        <div className="space-y-6 reveal-on-scroll">
          <h2 className="text-[11px] font-bold tracking-[0.8em] text-primary/60 uppercase">Ecosystem</h2>
          <h3 className="text-6xl md:text-8xl font-headline font-black tracking-tighter">Things I've Built</h3>
          <p className="text-2xl md:text-3xl text-muted-foreground max-w-2xl font-light leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 auto-rows-[350px]">
          {PRODUCTS.map((product, idx) => (
            <Link
              ...
              className={cn(
                "group relative glass rounded-[4rem] p-16 border-white/5 flex flex-col justify-between overflow-hidden transition-all duration-1000 hover:bg-[#3A506B]/40 hover:border-primary/20 reveal-on-scroll",
                product.className,
                `stagger-${(idx % 4) + 1}`
              )}
            >
              <div className="flex justify-between items-start relative z-10">
                <div className="w-20 h-20 rounded-3xl bg-white/[0.03] border border-white/5 flex items-center justify-center group-hover:scale-110 group-hover:bg-white/10 transition-all duration-700">
                  <product.icon className="w-10 h-10 text-muted-foreground group-hover:text-primary transition-colors" />
                </div>
                <div className="px-5 py-2 rounded-full glass border-white/5 text-[10px] font-bold tracking-[0.4em] uppercase text-muted-foreground group-hover:text-foreground transition-all">
                  {product.status}
                </div>
              </div>
              ...
              <div className="space-y-6 relative z-10">
                <div className="space-y-3">
                  <p className="text-[10px] font-bold tracking-[0.6em] text-primary uppercase">{product.tag}</p>
                  <p className="text-4xl md:text-5xl font-headline font-black text-foreground tracking-tighter">{product.title}</p>
                </div>
                <p className="text-muted-foreground text-xl leading-relaxed font-light opacity-0 group-hover:opacity-100 transition-all duration-1000 translate-y-6 group-hover:translate-y-0">
                  {product.desc}
                </p>
              </div>
