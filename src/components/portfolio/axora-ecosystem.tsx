    ...
    <section id="axora" className="py-64 px-6 relative overflow-hidden" ref={containerRef}>
      ...
      <div className="max-w-7xl mx-auto space-y-32">
        <div className="text-center space-y-10 reveal-on-scroll">
          <div className="inline-flex items-center gap-2 px-6 py-2 rounded-full glass border-primary/20 text-[11px] font-bold text-primary uppercase tracking-[0.6em]">Venture Studio</div>
          <h2 className="text-8xl md:text-[10rem] font-headline font-black tracking-tighter uppercase leading-none">AXORA</h2>
          <div className="max-w-3xl mx-auto space-y-8">
            ...
            <p className="text-xl text-primary/60 font-light italic">
        ...
        <div className="relative h-[600px] md:h-[900px] glass rounded-[5rem] border-white/5 overflow-hidden group">
          ...
          <div className="absolute inset-0 pointer-events-none">
             <svg className="w-full h-full">
               <defs>
                 <linearGradient id="lineGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                   <stop offset="0%" stopColor="rgba(200,217,230,0.1)" />
                   <stop offset="50%" stopColor="rgba(200,217,230,0.4)" />
                   <stop offset="100%" stopColor="rgba(200,217,230,0.1)" />
                 </linearGradient>
               </defs>
             ...
             <div className="absolute inset-0 overflow-hidden">
                {[1, 2, 3].map((p) => (
                  <div key={p} className={`absolute w-1 h-1 bg-primary rounded-full blur-[2px] opacity-40 animate-ping`} style={{ top: '50%', left: '50%' }} />
                ))}
             </div>
          </div>
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
             <div className="w-32 h-32 rounded-full glass border-primary/40 flex items-center justify-center shadow-[0_0_80px_rgba(200,217,230,0.15)] group-hover:scale-110 transition-transform duration-1000">
          ...
          {AXORA_PRODUCTS.map((product) => (
            ...
            >
              <div className={cn(
                ...
                <div className="w-20 h-20 rounded-3xl glass border-white/10 flex items-center justify-center transition-all duration-500 group-hover:border-primary/40 group-hover:shadow-[0_0_50px_rgba(200,217,230,0.2)]">
                  <product.icon className="w-8 h-8 text-muted-foreground group-hover:text-primary transition-colors" />
                </div>
                
                {/* Product Info Card */}
                <div className={cn(
                  "absolute top-full mt-8 left-1/2 -translate-x-1/2 w-64 glass p-8 rounded-3xl border-primary/20 text-center transition-all duration-700 pointer-events-none",
                  activeProduct === product.name ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                )}>
                  <p className="text-primary font-bold tracking-[0.4em] uppercase text-[10px] mb-3">{product.name}</p>
                  <p className="text-muted-foreground text-sm font-light leading-relaxed">{product.desc}</p>
                </div>
              </div>
            </div>
          ))}
